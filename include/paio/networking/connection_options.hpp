...
private:
    CommunicationType m_connection_type { paio::options::option_default_communication_type };
    std::string m_address {};
    int m_port { paio::options::option_default_port };
...
public:
    ...
    ConnectionOptions ()
    {
        ...
        switch (m_connection_type) {
            case CommunicationType::_unix:
                this->m_address = paio::options::option_default_socket_name ();
                break;

            case CommunicationType::inet:
                this->m_address = paio::options::option_default_address ();
                break;
     ...
     */
    ConnectionOptions (const CommunicationType& type, const std::string& address, const int& port) :
    ...
    {
        std::string message { "{" };
        message.append (this->connection_type_to_string (this->m_connection_type)).append (", ");
